...
import { findMusicBaseService,putFavouriteService } from '@/api/music'
const currentPage = ref(1)
const pageSize = ref(10)
const size = ref<ComponentSize>('default')
const emit = defineEmits(['play-song']);
const handleClick =function(music_url,rowdata){
...
const tableData=ref([])
const findMusicBaseServiceImpl=async function(){
    let parms={
    pageNum:currentPage.value,
    pagesize:pageSize.value
  }
  let result=await findMusicBaseService(parms);
...
const calcTableHeight = () => {
  ...
  if (container) {
    ...
    const padding = 40 
    tableMaxHeight.value = containerHeight - footerHeight - padding
  }
}
...
<template>
    ...
    <el-container class="main-container">
      <el-main class="table-content">
        ...
        <template #default="scope">
            <el-button :icon="VideoPlay" circle size="small" v-on:click="handleClick(scope.row.music_url,scope.row)"/>
            <el-button :icon="Star" circle size="small" v-on:click="putFavourite(scope.row.music_id)"/>
